...
make -j$JOBS install
# lipo -create node-64/default/node -output node
...
echo "Single-arch Node at $JAIL/bin/node"
...
git clone --depth=1 https://github.com/mapnik/mapnik.git -b statistics-tilemill mapnik
cd mapnik
mkdir osx
cd osx
...
git clone https://github.com/mapbox/tilemill.git -b statistics tilemill
cd tilemill


export PATH=$JAIL/bin:$PATH

export CORE_CXXFLAGS="-O3 -arch x86_64 -mmacosx-version-min=10.6 -isysroot /Developer/SDKs/MacOSX10.6.sdk"
export CORE_LINKFLAGS="-arch x86_64 -mmacosx-version-min=10.6 -isysroot /Developer/SDKs/MacOSX10.6.sdk"
...
npm install --force bones@1.3.20
npm install .
